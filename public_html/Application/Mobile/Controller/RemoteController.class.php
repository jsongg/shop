...
// echo $t;die;
		...
		// header('content-type:text/html;charset=utf-8');echo '<pre>';var_dump($data2);die;
		if($data2){
	...
	public function picpay(){
		...
		$res=json_decode($res,true);
		// echo '<pre>';var_dump($res);die;
		if(!empty($res['data'][0]['similarity'])){
			session('piclists',$res);
			
		}else{
			$this->error('<span style="color:red">支付失败</span> 没有搜到您的藏品信息');
			die;
		}
		...
		if($res['balance']<$price){
			$this->error('账户余额不足，请充值！',U('Mobile/Account/rechargeTc'));
			exit();
		}
